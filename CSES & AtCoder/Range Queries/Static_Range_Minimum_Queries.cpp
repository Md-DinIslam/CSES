...
int build(int s, int e, int rt) {
    if (s == e) return ans[rt] = v[s];
    int mid = (s + e) >> 1;
    return ans[rt] = min(build(s, mid, 2 * rt), build(mid + 1, e, 2 * rt + 1));
}
int query(int s, int e, int l, int r, int rt) {
    if (s > r || e < l) return 2e9;
    if (s >= l && e <= r) return ans[rt];
    int mid = (s + e) / 2;
    return min(query(s, mid, l, r, 2 * rt), query(mid + 1, e, l, r, 2 * rt + 1));
}
void solve()
{
    int n, q;
